...
- `apps/cli`: published CLI (`@chacelow-stack/create`), with source in `apps/cli/src` and tests in `apps/cli/test`.
...
Issues are tracked in GitHub Issues for `AmanVarshney01/@chacelow-stack/create` using the `gh` CLI. See `docs/agents/issue-tracker.md`.
