---
name: add-to-project
description: Add addons or features (PWA, Tauri, Starlight/Fumadocs docs, Biome/Oxlint, Husky/Lefthook, Turborepo/Nx, the MCP addon, etc.) to an existing Chacelow-Stack project. Use when the user wants to extend, enhance, or add tooling to a project that was created with Chacelow-Stack.
metadata:
  priority: 7
  docs:
...
# Add addons to an existing Chacelow-Stack project

Use the Chacelow-Stack MCP server to install addons into an existing project rather than wiring the tooling by hand.

## When this applies

The user already has a Chacelow-Stack project (look for a `bts.jsonc` config) and wants to add tooling or features — e.g. "add PWA support", "add a docs site", "switch to Biome", "add Turborepo", "wire up the MCP addon".

For brand-new projects, use the **scaffold-project** skill instead.

## Workflow

1. **Confirm the target project** is a Chacelow-Stack project and identify its directory.
