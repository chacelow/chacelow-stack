...
async function main(): Promise<void> {
  ...
  try {
    ...
    typesPubSpin.start(`Publishing @chacelow-stack/types@${canaryVersion} (canary)...`);
    try {
    ...
    templateGeneratorPackageJson.dependencies["@chacelow-stack/types"] = canaryVersion;
    await writeFile(
      TEMPLATE_GENERATOR_PACKAGE_JSON_PATH,
    ...
    templateGeneratorPubSpin.start(
      `Publishing @chacelow-stack/template-generator@${canaryVersion} (canary)...`,
    );
    try {
    ...
    packageJson.dependencies["@chacelow-stack/types"] = canaryVersion;
    packageJson.dependencies["@chacelow-stack/template-generator"] = canaryVersion;
    ...
    console.log(`📦 NPM: https://www.npmjs.com/package/@chacelow-stack/types/v/${canaryVersion}`);
    console.log(
      `📦 NPM: https://www.npmjs.com/package/@chacelow-stack/template-generator/v/${canaryVersion}`,
    );
  } finally {
    if (!restored) {
      await writeFile(CLI_PACKAGE_JSON_PATH, originalPackageJsonString);
